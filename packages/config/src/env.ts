...
function withFallbacks(raw: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  return {
    ...
    DATABASE_URL:
      raw.DATABASE_URL ||
      "postgresql://postgres:postgres@localhost:5432/msgflow",
    AUTH_SECRET:
      raw.AUTH_SECRET || "dev-only-auth-secret-not-for-production-use",
    WHATSAPP_WORKER_SECRET:
      raw.WHATSAPP_WORKER_SECRET || "dev-only-worker-secret",
    ENCRYPTION_KEY:
      raw.ENCRYPTION_KEY || "ZGV2LW9ubHktZW5jcnlwdGlvbi1rZXktMzJieXRlcyE=",
  };
}
