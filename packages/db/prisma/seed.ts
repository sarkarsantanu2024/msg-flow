...
async function main() {
  ...
  const plans = [
    {
      ...
      name: 'Starter',
      priceInr: 0,
      limits: { messagesPerMonth: 5_000, aiCallsPerMonth: 2_000, automations: 3, outputs: 3, seats: 3 },
      features: ['1 WhatsApp connection', 'Excel & CSV outputs', 'Real-time and daily processing'],
    },
    {
      slug: 'growth',
      name: 'Growth',
      priceInr: 4_999,
      limits: { messagesPerMonth: 50_000, aiCallsPerMonth: 25_000, automations: 25, outputs: 25, seats: 10 },
      features: ['Everything in Starter', 'Google Sheets & REST API outputs', 'Weekly and monthly reports', 'Version history'],
    },
    {
      slug: 'scale',
      name: 'Scale',
      priceInr: 14_999,
      limits: { messagesPerMonth: 500_000, aiCallsPerMonth: 250_000, automations: 200, outputs: 200, seats: 50 },
      features: ['Everything in Growth', 'Client website & admin sync', 'Priority support', 'Custom retention'],
    },
  ];
  ...
  const groupDefinitions = [
    { externalId: '120363000000000001@g.us', name: 'Sales Team', participants: 12 },
    { externalId: '120363000000000002@g.us', name: 'Inventory Updates', participants: 8 },
    { externalId: '120363000000000003@g.us', name: 'Dispatch & Delivery', participants: 15 },
    { externalId: '120363000000000004@g.us', name: 'Management', participants: 5 },
  ];
  ...
  for (const [index, definition] of groupDefinitions.entries()) {
    groups.push(
      await prisma.whatsAppGroup.upsert({
        where: { connectionId_externalId: { connectionId: connection.id, externalId: definition.externalId } },
        create: {
          tenantId: tenant.id,
  ...
  const salesSchema = await prisma.extractionSchema.upsert({
    ...
    create: {
      ...
      fields: {
        create: [
          { key: 'date', label: 'Date', type: 'DATE', required: true, isKeyField: true, order: 0, description: 'Date of the enquiry' },
          { key: 'customerName', label: 'Customer', type: 'STRING', required: true, isKeyField: true, order: 1, description: 'Customer or company name' },
          { key: 'product', label: 'Product', type: 'STRING', required: true, isKeyField: true, order: 2, description: 'Product enquired about' },
          { key: 'quantity', label: 'Quantity', type: 'DECIMAL', required: false, order: 3, description: 'Quantity requested' },
          { key: 'unit', label: 'Unit', type: 'STRING', required: false, order: 4, description: 'Unit of measure' },
          { key: 'rate', label: 'Rate', type: 'CURRENCY', required: false, order: 5, description: 'Rate per unit in INR' },
          { key: 'salesPerson', label: 'Sales Person', type: 'STRING', required: false, order: 6, description: 'Who reported it' },
        ],
      },
    },
    update: {},
  ...
  const inventorySchema = await prisma.extractionSchema.upsert({
    ...
    create: {
      ...
      fields: {
        create: [
          { key: 'product', label: 'Product', type: 'STRING', required: true, isKeyField: true, order: 0 },
          { key: 'stock', label: 'Stock', type: 'DECIMAL', required: true, order: 1 },
          { key: 'unit', label: 'Unit', type: 'STRING', required: false, order: 2 },
          { key: 'location', label: 'Location', type: 'STRING', required: false, order: 3 },
          { key: 'date', label: 'Date', type: 'DATE', required: false, order: 4 },
        ],
      },
    },
    update: {},
  ...
  const deliverySchema = await prisma.extractionSchema.upsert({
    ...
    create: {
      ...
      fields: {
        create: [
          { key: 'orderId', label: 'Order ID', type: 'STRING', required: true, isKeyField: true, order: 0 },
          { key: 'status', label: 'Status', type: 'ENUM', required: false, order: 1, enumValues: ['Dispatched', 'In transit', 'Delivered', 'Delayed'] },
          { key: 'customerName', label: 'Customer', type: 'STRING', required: false, order: 2 },
          { key: 'vehicle', label: 'Vehicle / LR', type: 'STRING', required: false, order: 3 },
          { key: 'date', label: 'Date', type: 'DATE', required: false, order: 4 },
        ],
      },
    },
  ...
  const messageSets = [
    ...
    { group: groups[1], texts: INVENTORY_MESSAGES, category: 'INVENTORY' as const },
    { group: groups[2], texts: DELIVERY_MESSAGES, category: 'DELIVERY' as const },
  ];

  const createdMessages: Array<{ id: string; text: string; groupIndex: number; timestamp: Date }> = [];
  ...
  for (const [setIndex, set] of messageSets.entries()) {
    for (const [index, text] of set.texts.entries()) {
      ...
      const timestamp = daysAgo(6 - Math.floor(index / 2), 9 + (index % 8), (index * 13) % 60);
      ...
      const message = await prisma.message.upsert({
        ...
        create: {
          ...
          externalId,
          contentHash: sha256(`${set.group.externalId}|${sender.id}|${timestamp.getTime()}|${text}`),
          senderId: sender.id,
      ...
      if (!isNoise) {
        createdMessages.push({ id: message.id, text, groupIndex: setIndex, timestamp });
      }
    }
  }
  ...
  const masterExcel = await prisma.output.upsert({
    ...
    create: {
      ...
      config: {
        ...
        worksheet: 'Sales',
        headerRow: 1,
        columns: ['Date', 'Customer', 'Product', 'Quantity', 'Unit', 'Rate', 'Salesperson'],
  ...
  const dailyReport = await prisma.output.upsert({
    ...
    create: {
      ...
      type: 'PDF',
      status: 'ACTIVE',
      config: { fileName: 'daily-sales', title: 'Daily Sales Report', columns: [] } as Prisma.InputJsonValue,
      createdBy: owner.id,
    },
    update: {},
  });
  ...
  const salesAutomation = await prisma.automation.upsert({
    ...
    create: {
      ...
      description: 'Reads the Sales Team group and keeps Master Sales.xlsx up to date.',
      status: 'ACTIVE',
      ...
      createdBy: owner.id,
      triggers: {
        create: [{ tenantId: tenant.id, type: 'REAL_TIME', groupId: groups[0].id, enabled: true }],
      },
    },
    update: {},
  });
  ...
  const inventoryAutomation = await prisma.automation.upsert({
    ...
    create: {
      ...
      tenantId: tenant.id,
      name: 'Inventory Sync',
      description: 'Keeps the live stock sheet in step with what the team reports.',
      status: 'ACTIVE',
      ...
      createdBy: owner.id,
      triggers: {
        create: [{ tenantId: tenant.id, type: 'SCHEDULE', groupId: groups[1].id, enabled: true }],
      },
    },
    update: {},
  });
  ...
  await prisma.automation.upsert({
    ...
    create: {
      ...
      createdBy: owner.id,
      triggers: {
        create: [{ tenantId: tenant.id, type: 'REAL_TIME', groupId: groups[2].id, enabled: true }],
      },
    },
    update: {},
  });
  ...
  await prisma.outputTarget.upsert({
    where: { automationId_outputId: { automationId: salesAutomation.id, outputId: masterExcel.id } },
    create: {
      tenantId: tenant.id,
      ...
      mappings: {
        create: [
          { tenantId: tenant.id, sourceField: 'date', targetField: 'Date', isKeyPart: true, keyOrder: 0, order: 0, updateStrategy: 'NEVER_UPDATE' },
          { tenantId: tenant.id, sourceField: 'customerName', targetField: 'Customer', isKeyPart: true, keyOrder: 1, order: 1, updateStrategy: 'NEVER_UPDATE' },
          { tenantId: tenant.id, sourceField: 'product', targetField: 'Product', isKeyPart: true, keyOrder: 2, order: 2, updateStrategy: 'NEVER_UPDATE' },
          { tenantId: tenant.id, sourceField: 'quantity', targetField: 'Quantity', order: 3, updateStrategy: 'ALWAYS_UPDATE' },
          { tenantId: tenant.id, sourceField: 'unit', targetField: 'Unit', order: 4, updateStrategy: 'UPDATE_IF_EMPTY' },
          { tenantId: tenant.id, sourceField: 'rate', targetField: 'Rate', order: 5, updateStrategy: 'ALWAYS_UPDATE' },
          { tenantId: tenant.id, sourceField: 'salesPerson', targetField: 'Salesperson', order: 6, updateStrategy: 'UPDATE_IF_EMPTY' },
        ],
      },
    },
    update: {},
  ...
  await prisma.outputTarget.upsert({
    where: { automationId_outputId: { automationId: salesAutomation.id, outputId: dailyReport.id } },
    create: {
      tenantId: tenant.id,
      automationId: salesAutomation.id,
      ...
      mappings: {
        create: [
          { tenantId: tenant.id, sourceField: 'date', targetField: 'Date', order: 0 },
          { tenantId: tenant.id, sourceField: 'customerName', targetField: 'Customer', order: 1 },
          { tenantId: tenant.id, sourceField: 'product', targetField: 'Product', order: 2 },
          { tenantId: tenant.id, sourceField: 'quantity', targetField: 'Quantity', order: 3 },
          { tenantId: tenant.id, sourceField: 'rate', targetField: 'Rate', order: 4 },
        ],
      },
    },
    update: {},
  ...
  await prisma.outputTarget.upsert({
    where: { automationId_outputId: { automationId: inventoryAutomation.id, outputId: stockSheet.id } },
    create: {
      tenantId: tenant.id,
      ...
      mappings: {
        create: [
          { tenantId: tenant.id, sourceField: 'product', targetField: 'Product', isKeyPart: true, keyOrder: 0, order: 0, updateStrategy: 'NEVER_UPDATE' },
          { tenantId: tenant.id, sourceField: 'stock', targetField: 'Stock', order: 1, updateStrategy: 'ALWAYS_UPDATE' },
          { tenantId: tenant.id, sourceField: 'unit', targetField: 'Unit', order: 2, updateStrategy: 'UPDATE_IF_EMPTY' },
          { tenantId: tenant.id, sourceField: 'location', targetField: 'Location', order: 3, updateStrategy: 'ALWAYS_UPDATE' },
          { tenantId: tenant.id, sourceField: 'date', targetField: 'Updated', order: 4, updateStrategy: 'ALWAYS_UPDATE' },
        ],
      },
    },
    update: {},
  ...
  // ---- Extracted records ---------------------------------------------------
  const salesRecords = [
    { date: '2026-08-08', customerName: 'ABC Traders', product: 'Product X', quantity: 75, unit: 'kg', rate: 250, salesPerson: 'Rahul Sharma', confidence: 0.93 },
    { date: '2026-08-08', customerName: 'Sunrise Enterprises', product: 'Product Y', quantity: 120, unit: 'pcs', rate: 180, salesPerson: 'Priya Nair', confidence: 0.9 },
    { date: '2026-08-09', customerName: 'Metro Supplies', product: 'Product X', quantity: 30, unit: 'kg', salesPerson: 'Amit Patel', confidence: 0.71 },
    { date: '2026-08-10', customerName: 'Kumar Industries', product: 'Product Z', quantity: 200, unit: 'pcs', salesPerson: 'Rahul Sharma', confidence: 0.64 },
    { date: '2026-08-11', customerName: 'Deepak Trading', product: 'Product Y', quantity: 60, unit: 'kg', rate: 195, salesPerson: 'Priya Nair', confidence: 0.88 },
  ];
  ...
  for (const data of salesRecords) {
    ...
    await prisma.outputSyncRecord.upsert({
      where: { outputId_recordId: { outputId: masterExcel.id, recordId: record.id } },
      create: {
        tenantId: tenant.id,
        outputId: masterExcel.id,
        recordId: record.id,
  ...
  const inventoryRecords = [
    { product: 'Product X', stock: 275, unit: 'kg', location: 'Main godown', date: '2026-08-11', confidence: 0.91 },
    { product: 'Product Y', stock: 45, unit: 'pcs', location: 'Godown 2', date: '2026-08-11', confidence: 0.87 },
    { product: 'Product Z', stock: 0, unit: 'pcs', location: 'Main godown', date: '2026-08-10', confidence: 0.82 },
  ];
  ...
  console.log(`✓ ${salesRecords.length + inventoryRecords.length} extracted records (1 needs review)`);

  await prisma.output.update({
    where: { id: masterExcel.id },
    data: { recordCount: salesRecords.length, lastSyncAt: daysAgo(0, 8, 5), lastSyncStatus: 'SUCCESS' },
  });
  ...
  for (let day = 6; day >= 0; day--) {
    ...
    await prisma.workflowRun.upsert({
      ...
      create: {
        ...
        errorMessage: failed ? 'One row could not be written: the destination column "Rate" was locked.' : null,
        summary: {
          windowLabel: 'Since last successful run',
          outputs: [{ name: 'Master Sales', status: failed ? 'PARTIAL_SUCCESS' : 'SUCCESS', created: day % 3, updated: 1 + (day % 2), skipped: 2, failed: failed ? 1 : 0 }],
          warnings: [],
        } as Prisma.InputJsonValue,
